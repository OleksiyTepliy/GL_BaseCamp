...
const uint8_t key_in_mem[] PROGMEM = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; // default key value
...
ow_err ow_tx_reset(ow_Pin *pin)
{
	...
	uint16_t us_left = 390 - 60 - wait;					/****		25 release + 100 pres + 380 left		****/
...
// арбитраж, считываем значение линии 8 раз.
ow_err __ow_rx_bit(ow_Pin *pin, bool *bit, bool fastfail)
{
	...
	(*pin->line_low)();
	(*pin->delay_us)(2);						// pin->ts_holdlow_us
	(*pin->line_release)();
	(*pin->delay_us)(5);						//15 - pin->ts_holdlow_us
	...
	*bit = (cnt >= 5);
	...
	if (!err || !fastfail) {
		(*pin->delay_us)(20);					//pin->ts_len_us - 15
...
ow_err __ow_cmd_readrom(ow_Pin *pin, uint8_t *rombytes, uint8_t *devcrc, bool resetfirst, bool fastfail)
{
	...
	// uint8_t data[80];
	// sprintf((char *) data, "\n fam_code = %#X\n readcrc = %#X\n calccrc = %#X\n ", 
	// 		rombytes[0], readcrc, calccrc);
	// uart_put(data);


	/* SERIAL = %#X %#X %#X %#X %#X %#X\n  rombytes[1], rombytes[2], rombytes[3], 
	rombytes[4], rombytes[5], rombytes[6] */
